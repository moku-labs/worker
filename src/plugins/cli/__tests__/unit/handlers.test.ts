 ...
 * Verifies the three TUI formatter handlers print the expected lines via ctx.log.info.
 */
...
describe("createCliHooks", () => {
  ...
  describe("deploy:complete handler", () => {
    it('logs "done -> <url>" with the deployed URL', () => {
      const { ctx, logInfo } = makeMockCtx();
      const hooks = createCliHooks(ctx);

      hooks["deploy:complete"]({ url: "https://my-worker.workers.dev" });

      expect(logInfo).toHaveBeenCalledWith("done -> https://my-worker.workers.dev");
    });
