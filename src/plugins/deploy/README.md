...
**Throws:** only in `ci` / non-interactive runs (fail-fast). The first failure — auth, infra preflight, provisioning, upload, or the wrangler subprocess (`[worker] wrangler exited with code <n>` / `[worker] Failed to spawn wrangler`) — propagates to the caller (the `cli` plugin renders it as a branded `✗` + non-zero exit).
